...
class EntelAuth extends EntelDB {
	...
	function login($username, $password, $ajax = TRUE) {
		...
		if ($stmt -> rowCount() == 0) {
			$_SESSION['user_allow'] = FALSE;
			//echo json_encode(array($this -> loginQuery));
			if ($ajax) {
			...
			if ($ajax) {
				$feedback = array("login" => "success", "message" => "Login successful", "passURL" => $this -> loginpassRedir);
				echo json_encode($feedback);
			} else {
	...
	function displayUsernameNavbar() {
		if (isset($_SESSION['user_allow']) and isset($_SESSION['user'])){
